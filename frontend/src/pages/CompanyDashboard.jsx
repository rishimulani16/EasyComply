...
export default function CompanyDashboard() {
    ...
    const sortRows = (rows) => {
        ...
        if (sortBy === 'penaltyImpact') {
            copy.sort((a, b) => {
                const aRank = IMPACT_ORDER[a.penalty_impact] ?? 4;
                const bRank = IMPACT_ORDER[b.penalty_impact] ?? 4;
                return aRank - bRank;
            });
        } else {
    ...
    // ── Sort dropdown (shown in each panel header) ────────────────────────
    const SortBar = () => (
                        ...
                        <div className="space-y-6">
                            ...
                            {companyWide.length > 0 && (
                                <div className="rounded-2xl border border-slate-600/50 overflow-hidden">
                                    ...
                                    <div className="bg-slate-800/40">
                                        <RulesTable rows={sortRows(companyWide)} />
                                    </div>
                                </div>
                            )}
                        </div>
